use crate::{
    ...
    markets::{
        meteora::fetch_new_meteora_pools, orca_whirpools::fetch_new_orca_whirpools,
        raydium::fetch_new_raydium_pools, types::Market,
    },
};
use log::info;
...
pub async fn get_fresh_pools(tokens: Vec<TokenInArb>) -> HashMap<String, Market> {
    let env = Env::new();
    let rpc_client =
        RpcClient::new_with_commitment(env.rpc_url.clone(), CommitmentConfig::confirmed());
    let mut new_markets: HashMap<String, Market> = HashMap::new();
    let mut _count_new_pools = 0;
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        let orca_res_tokena =
            fetch_new_orca_whirpools(&rpc_client, token.token.clone(), false).await;
        ...
        let orca_res_tokenb =
            fetch_new_orca_whirpools(&rpc_client, token.token.clone(), true).await;
        ...
        //Raydium Markets
        let raydium_res_tokena =
            fetch_new_raydium_pools(&rpc_client, token.token.clone(), false).await;
        ...
        sleep(Duration::from_millis(2000)).await;
        println!("1 GetProgramAccounts Raydium");
        let raydium_res_tokenb =
            fetch_new_raydium_pools(&rpc_client, token.token.clone(), true).await;
        ...
        //Meteora Markets
        let meteora_res_tokena =
            fetch_new_meteora_pools(&rpc_client, token.token.clone(), false).await;
        ...
        sleep(Duration::from_millis(2000)).await;
        println!("1 GetProgramAccounts Meteora");
        let meteora_res_tokenb =
            fetch_new_meteora_pools(&rpc_client, token.token.clone(), true).await;

...
pub async fn get_fresh_pools(tokens: Vec<TokenInArb>) -> HashMap<String, Market> {
    let env = Env::new();
    let rpc_client =
        RpcClient::new_with_commitment(env.rpc_url.as_str(), CommitmentConfig::confirmed());
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        let orca_res_tokena =
            fetch_new_orca_whirpools(&rpc_client, token.address.clone(), false).await;
        ...
        let orca_res_tokenb =
            fetch_new_orca_whirpools(&rpc_client, token.address.clone(), true).await;
        ...
        let raydium_res_tokena =
            fetch_new_raydium_pools(&rpc_client, token.address.clone(), false).await;
        ...
        let raydium_res_tokenb =
            fetch_new_raydium_pools(&rpc_client, token.address.clone(), true).await;
        ...
        let meteora_res_tokena =
            fetch_new_meteora_pools(&rpc_client, token.address.clone(), false).await;
        ...
        let meteora_res_tokenb =
            fetch_new_meteora_pools(&rpc_client, token.address.clone(), true).await;

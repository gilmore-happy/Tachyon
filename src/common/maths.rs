...
pub fn from_x64_orca_wp(num: u128, decimals_0: f64, decimals_1: f64) -> Decimal {
    ...
    let from_x64: Decimal = num_dec
        .checked_mul(Decimal::from_f64_retain(mul_x64).unwrap())
        .unwrap();
    let price: Option<Decimal> = from_x64
        .checked_powd(dec!(2))
        .unwrap()
        .checked_mul(Decimal::from_f64_retain(10f64.powf(decimals_0 - decimals_1)).unwrap());
    return price.unwrap();

...
impl DynamoDBClient {
    ...
    pub async fn insert_swap_path_result(&self, table_name: &str, sp_result: SwapPathResult) -> Result<()> {
        // Convert the struct to a JSON string then to DynamoDB AttributeValues
        let json_string = serde_json::to_string(&sp_result)?;
        ...
        item.insert(
            ...
            AttributeValue::S(format!("{}-{}", sp_result.token_in_symbol, sp_result.token_out_symbol)),
        );
        ...
        info!("📊 SwapPathResult written to DynamoDB table: {}", table_name);
        Ok(())
    }

    pub async fn insert_vec_swap_path_selected(&self, table_name: &str, best_paths: VecSwapPathSelected) -> Result<()> {
        ...
        for (index, path) in best_paths.value.iter().enumerate() {
            ...
            item.insert(
                ...
                AttributeValue::S(format!("selection_{}", chrono::Utc::now().timestamp_millis())),
            );
            item.insert(
        ...
        info!("📊 VecSwapPathSelected written to DynamoDB table: {}", table_name);
        Ok(())
    }
...
pub async fn insert_swap_path_result_collection(table_name: &str, sp_result: SwapPathResult) -> Result<()> {
    let client = DynamoDBClient::new().await?;
    client.insert_swap_path_result(table_name, sp_result).await
}

pub async fn insert_vec_swap_path_selected_collection(table_name: &str, best_paths_for_strat: VecSwapPathSelected) -> Result<()> {
    let client = DynamoDBClient::new().await?;
    client.insert_vec_swap_path_selected(table_name, best_paths_for_strat).await
}

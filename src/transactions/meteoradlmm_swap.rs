...
//
pub async fn construct_meteora_instructions(
    params: SwapParametersMeteora,
    ...
    let (event_authority, _bump) =
        Pubkey::find_program_address(&[b"__event_authority"], &amm_program);
    ...
    let bin_array_2 =
        derive_bin_array_pda(lb_pair, (active_bin_array_idx - 2) as i64, amm_program).0;
...
pub fn derive_bin_array_bitmap_extension(lb_pair: Pubkey, amm_program: Pubkey) -> (Pubkey, u8) {
    let result =
        Pubkey::find_program_address(&[BIN_ARRAY_BITMAP_SEED, lb_pair.as_ref()], &amm_program);
    return result;
}

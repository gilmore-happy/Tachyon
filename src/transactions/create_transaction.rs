...
pub async fn create_and_send_swap_transaction(simulate_or_send: SendOrSimulate, chain: ChainType, transaction_infos: SwapPathResult) -> Result<()> {
    ...
    create_and_send_swap_transaction_with_fee(simulate_or_send, chain, transaction_infos, priority_fee).await
}
...
pub async fn create_and_send_swap_transaction_with_fee(
    ...
    info!("🔄 Create swap transaction with priority fee: {} microlamports ({:.6} SOL)",
    ...
    let compute_budget_instruction_details = InstructionDetails{ instruction: compute_budget_ix, details: "Compute Budget Instruction".to_string(), market: None };
    
    let priority_fees_ix = ComputeBudgetInstruction::set_compute_unit_price(priority_fee_microlamports);
    let priority_fees_instruction_details = InstructionDetails{ instruction: priority_fees_ix, details: format!("Set priority fees: {} microlamports", priority_fee_microlamports), market: None };

    let swaps_construct_instructions: Vec<InstructionDetails> = construct_transaction(transaction_infos.clone()).await;
    // Order: Priority Fee, Compute Budget, Swap Instructions
    let mut swap_instructions_details: Vec<InstructionDetails> = vec![priority_fees_instruction_details, compute_budget_instruction_details];
    swap_instructions_details.extend(swaps_construct_instructions);


    if swap_instructions_details.iter().skip(2).all(|id| id.instruction.accounts.is_empty() && id.instruction.data.is_empty()) { // Check if actual swap instructions are empty
    ...
    for si_detail in swap_instructions_details.iter() { // Iterate over details
        if let Some(market_addr) = si_detail.market.as_ref().map(|m| m.address) {
            match get_lut_address_for_market(market_addr, false) {
                ...
                Ok((_, None)) => {
                     error!("❌ No LUT address found for the market {:?}, the tx can revert...", market_addr);
                }
                Err(e) => {
                    error!("❌ Error getting LUT address for market {:?}: {:?}", market_addr, e);
                }
            }
        } else {
            info!("Skip get LUT table for non swap instruction: {:?}", si_detail.details);
        }
    }
    
    let si_details_log: Vec<String> = swap_instructions_details.iter().map(|instruc_details| instruc_details.details.clone()).collect();
    ...
    for lut_address in lut_addresses.iter() { // Iterate over references
        match rpc_client.get_account(lut_address) {
            Ok(raw_lut_account) => {
                match AddressLookupTable::deserialize(&raw_lut_account.data) {
                    Ok(address_lookup_table) => {
                        let address_lookup_table_account = AddressLookupTableAccount {
                            key: *lut_address,
                            addresses: address_lookup_table.addresses.to_vec(),
                        };
                        info!("Address in lookup_table {}: {} addresses", lut_address, address_lookup_table_account.addresses.len());
    ...
    let mut instructions_for_tx: Vec<Instruction> = swap_instructions_details.iter().map(|instruc_details| instruc_details.instruction.clone()).collect();
    ...
    if instructions_for_tx.len() >= 2 {
        instructions_for_tx[0] = ComputeBudgetInstruction::set_compute_unit_price(priority_fee_microlamports);
        instructions_for_tx[1] = ComputeBudgetInstruction::set_compute_unit_limit(consumed_cus as u32);
    } else {
        ...
        let new_priority_ix = ComputeBudgetInstruction::set_compute_unit_price(priority_fee_microlamports);
        let new_cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(consumed_cus as u32);
        let original_swaps: Vec<Instruction> = swap_instructions_details.iter().skip(2).map(|id| id.instruction.clone()).collect();
    ...
    if simulate_or_send == SendOrSimulate::Send {
        ...
        let new_payer_for_send: Keypair = read_keypair_file(&env.payer_keypair_path).expect("Wallet keypair file not found for send");
        
        let latest_blockhash_for_send = match rpc_client.get_latest_blockhash_with_commitment(commitment_config) {
             Ok((hash, _)) => hash,
             Err(e) => {
        ...
        let versioned_tx_to_send = VersionedTransaction::try_new(message_to_send, &[&new_payer_for_send])?;

        info!("📡 Sending transaction with RPC: {}", rpc_client.url());
        
        let non_blocking_rpc_client_send = solana_client::nonblocking::rpc_client::RpcClient::new(env.rpc_url_tx.clone());
        ...
        if let ConnectionCache::Quic(cache_send) = connection_cache_send {
            match solana_client::nonblocking::tpu_client::TpuClient::new_with_connection_cache(
                ...
                Ok(tpu_client) => {
                    ...
                    match arc_rpc_client_send.send_transaction_with_config(&versioned_tx_to_send, transaction_send_config).await {
                        Ok(signature) => {
                            info!("✅ Transaction sent via TPU/RPC with signature: {}", signature);
                ...
                Err(e) => {
                     error!("❌ Failed to create TPU client: {:?}. Falling back to RPC send.", e);
                     // Fallback to RPC send if TPU client creation fails
                    match arc_rpc_client_send.send_transaction_with_config(&versioned_tx_to_send, transaction_send_config).await {
                        Ok(signature) => info!("✅ Transaction sent via RPC fallback with signature: {}", signature),
                        Err(rpc_e) => error!("❌ Failed to send transaction via RPC fallback: {:?}", rpc_e),
                    }
                }
            }
        } else {
            warn!("⚠️ Using basic RPC send_transaction for non-QUIC endpoint. Consider QUIC for TPU.");
            match arc_rpc_client_send.send_transaction_with_config(&versioned_tx_to_send, transaction_send_config).await {
...
pub async fn create_ata_extendlut_transaction(chain: ChainType, simulate_or_send: SendOrSimulate, transaction_infos: SwapPathResult, lut_address: Pubkey, tokens: Vec<Pubkey>) -> Result<()> {
    ...
    for token in tokens {
        ...
        match rpc_client.get_account(&pda_user_token) {
            Ok(_account) => { // Changed variable name
                info!("🟢 PDA for {} already exist !", token);
            }
            Err(_error) => { // Changed variable name
    ...
    let mut swap_instructions_for_lut: Vec<InstructionDetails> = construct_transaction(transaction_infos.clone()).await; // Cloned
    
    let mut instructions_to_extend_lut: Vec<InstructionDetails> = Vec::new();
    for instruction_detail in swap_instructions_for_lut.iter_mut() { // Iterate mutably
        if let Some(market_info) = instruction_detail.market.as_ref() {
            let market_addr = market_info.address;
            match get_lut_address_for_market(market_addr, false) {
                Ok((lut_exist, Some(_))) if lut_exist => { // Check if lut_address is Some
                ...
                _ => {  
                    ...
                    if !instruction_detail.instruction.accounts.is_empty() {
                         instructions_to_extend_lut.push(instruction_detail.clone()); // Clone if adding
                    }
                }
            }
        }
    ...
    if let Some(instr_to_extend) = instructions_to_extend_lut.first() {
        if !instr_to_extend.instruction.accounts.is_empty() {
            let accounts_to_add: Vec<Pubkey> = instr_to_extend.instruction.accounts.iter().map(|acc_meta| acc_meta.pubkey).collect();
            if !accounts_to_add.is_empty() {
                ...
                vec_details_extend_instructions_final.push(InstructionDetails{
                    ...
                    details: format!("Extend LUT {} for market {:?}", lut_address, instr_to_extend.market.as_ref().map(|m|m.address)), 
                    market: instr_to_extend.market.clone()
                });
                info!("Extend LUT instruction prepared for market: {:?}", instr_to_extend.market.as_ref().map(|m|m.address));
            }
        }
    }
    ...
    let priority_fees_ix_lut = ComputeBudgetInstruction::set_compute_unit_price(env.priority_fee_lut.unwrap_or(100)); // Use env var or default
    
    let mut vec_all_instructions_lut: Vec<Instruction> = vec![priority_fees_ix_lut, compute_budget_ix_lut];
    vec_all_instructions_lut.extend(vec_pda_instructions.clone());
    vec_all_instructions_lut.extend(vec_details_extend_instructions_final.iter().map(|id| id.instruction.clone()));


    if vec_all_instructions_lut.len() <= 2 { // Only CU and priority fee
        info!("➡️ No actual ATA creation or LUT extension instructions to send.");
        return Ok(());
    }

    let commitment_config_lut = CommitmentConfig::confirmed();
    let latest_blockhash_lut = match rpc_client.get_latest_blockhash_with_commitment(commitment_config_lut) {
        Ok((hash, _)) => hash,
        Err(e) => {
    ...
    let txn_simulate_lut = VersionedTransaction::try_new(VersionedMessage::V0(txn_simulate_lut_msg), &[&payer])?;
    ...
    let sim_result_lut = match rpc_client.simulate_transaction_with_config(&txn_simulate_lut, sim_config_lut) {
        Ok(response) => response.value,
        Err(e) => {
    ...
    if sim_result_lut.err.is_some() {
        error!("❌ Get out! Simulate Error for LUT/ATA Tx: {:#?}", sim_result_lut.err);
        info!("📜 LUT/ATA Simulation Logs on error: {:#?}", sim_result_lut.logs);
        return Ok(());
    } else {
    ...
    vec_all_instructions_lut[1] = ComputeBudgetInstruction::set_compute_unit_limit(result_cu_lut as u32);
    ...
    if simulate_or_send == SendOrSimulate::Send {
        ...
        let new_payer_lut: Keypair = read_keypair_file(&env.payer_keypair_path).expect("Wallet keypair file not found for LUT send");
        
        let latest_blockhash_lut_send = match rpc_client.get_latest_blockhash_with_commitment(commitment_config_lut) {
            Ok((hash, _)) => hash,
            Err(e) => {
        ...
        info!("📡 Sending LUT/ATA transaction via RPC: {}", rpc_client.url());
                ...
                if check_tx_status(commitment_config_lut, chain, signature).await? {
                    for details_instruction_written in vec_details_extend_instructions_final {
                        if let Some(market_info_written) = details_instruction_written.market {
                             match write_lut_for_market(market_info_written.address, lut_address, false) {
                                Ok(_) => info!("Successfully wrote LUT info for market {}", market_info_written.address),
                                Err(e) => error!("Failed to write LUT info for market {}: {:?}", market_info_written.address, e),
                             }
                        }
                    }
        ...
        match route_sim.dex_label {
            DexLabel::METEORA => {
                let swap_params: SwapParametersMeteora = SwapParametersMeteora{
                    ...
                    minimum_amount_out: transaction_infos.route_simulations[i].estimated_amount_out.parse().unwrap_or_else(|_| {
                        error!("Failed to parse minimum_amount_out for Meteora: {}", transaction_infos.route_simulations[i].estimated_amount_out);
                        0 // Fallback, consider how to handle this better
                    })
                };
                let result = construct_meteora_instructions(swap_params.clone()).await;
                if result.is_empty() { // Check if empty
            ...
            DexLabel::RAYDIUM => {
                let swap_params: SwapParametersRaydium = SwapParametersRaydium{
                    ...
                    min_amount_out: transaction_infos.route_simulations[i].estimated_amount_out.parse().unwrap_or_else(|_| {
                        error!("Failed to parse minimum_amount_out for Raydium: {}", transaction_infos.route_simulations[i].estimated_amount_out);
                        0 
                    })
                };
            ...
            DexLabel::ORCA_WHIRLPOOLS => {
                let swap_params: SwapParametersOrcaWhirpools = SwapParametersOrcaWhirpools{
                    ...
                    amount_in: transaction_infos.route_simulations[i].amount_in,
                    minimum_amount_out: transaction_infos.route_simulations[i].estimated_amount_out.parse().unwrap_or_else(|_| {
                        error!("Failed to parse minimum_amount_out for Orca: {}", transaction_infos.route_simulations[i].estimated_amount_out);
                        0
                    })
                };
...
pub async fn create_lut(chain: ChainType) -> Result<()> {
    ...
    let env = Env::new();
    let rpc_url = if chain == ChainType::Mainnet { env.rpc_url.clone() } else { env.devnet_rpc_url.clone() }; // Cloned
    let rpc_client: RpcClient = RpcClient::new(rpc_url);
    let payer: Keypair = read_keypair_file(&env.payer_keypair_path).expect("Wallet keypair file not found"); // Removed clone
    ...
    let latest_blockhash_lut = rpc_client.get_latest_blockhash().expect("Error in get latest blockhash for LUT");

    let txn_lut_msg = v0::Message::try_compile(&payer.pubkey(), &[create_lut_instruction.clone()], &[], latest_blockhash_lut)?;
    ...
    if tx_confirmed {
        ...
        error!("❌ Address LUT {} creation failed or not confirmed", lut_address);
    }
...
pub async fn is_available_lut(chain: ChainType, lut_address: Pubkey) -> Result<bool> {
    info!("🚚 Check if LUT address {} is available to extend...", lut_address);
    let env = Env::new();
    let rpc_url = if chain == ChainType::Mainnet { env.rpc_url.clone() } else { env.devnet_rpc_url.clone() };
    let rpc_client: RpcClient = RpcClient::new(rpc_url);
    ...
    if current_len < MAX_ADDRESSES_PER_LUT - env.lut_buffer_count.unwrap_or(10) as usize { // Use env var or default buffer
        info!("LUT {} has {} addresses, available for extension.", lut_address, current_len);
        return Ok(true)
    } else {
        info!("LUT {} has {} addresses, considered full or close to full.", lut_address, current_len);
        return Ok(false)
    }
}
...
pub fn get_lut_address_for_market(market: Pubkey, is_test: bool) -> Result<(bool, Option<Pubkey>)> {
    ...
    let lut_file_data: VecLUTFile = match serde_json::from_reader(reader) {
        Ok(data) => data,
        Err(e) if e.is_eof() => { // Handle empty file case
    ...
    let found_entry = lut_file_data.value.iter().find(|entry| entry.market == market_str);
    ...
    match found_entry {
        Some(value) => {
            match from_str(&value.lut_address) {
                ...
                Err(e) => {
                    error!("Failed to parse Pubkey from LUT address string '{}' for market {}: {:?}", value.lut_address, market_str, e);
...
pub fn write_lut_for_market(market:Pubkey, lut_address: Pubkey, is_test: bool) -> Result<()> {
    ...
    if let Some(entry) = lut_file_data.value.iter_mut().find(|e| e.market == market_str) {
        info!("Updating LUT address for market {} from {} to {}", market_str, entry.lut_address, lut_address_str);
        entry.lut_address = lut_address_str;
    } else {
        info!("Adding new LUT entry for market {}: {}", market_str, lut_address_str);
        lut_file_data.value.push(LUTFile{ market: market_str, lut_address: lut_address_str });
    }
    ...
    let file_write = OpenOptions::new().write(true).create(true).truncate(true).open(path)?;

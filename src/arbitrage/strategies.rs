...
use crate::execution::executor::{ExecutionQueue, execute_profitable_swap, ExecutionMode};
        ...
        if swap_simulation_result.len() >= path.hops as usize {
            ...
            if let Some(queue) = execution_queue {
                if result_difference > 20000000.0 {
                    ...
                    info!("💸 Sending to execution queue: {} SOL profit", result_difference / 1e9);
            ...
            //Code to avoid simulations of all paths on min. 3 likely consecutive failed paths
...
pub async fn optimism_tx_strategy(path: String, execution_queue: Option<&ExecutionQueue>) -> Result<()> {
    let file_read = OpenOptions::new().read(true).write(true).open(path)?;
